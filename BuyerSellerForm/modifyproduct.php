...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['e1'], $_POST['f1'],$_POST['p1'])) {
        $product_id = $_POST['e1'];
        $product_des = $_POST['f1'];
        $product_qty = $_POST['p1'];

        $check_query = "SELECT * FROM product WHERE ProID = '$product_id'";
        $result = mysqli_query($con, $check_query);

        if (mysqli_num_rows($result) > 0) {
            $update_query = "UPDATE product 
                            SET ProDes = '$product_des',
                                ProQty ='$product_qty'
